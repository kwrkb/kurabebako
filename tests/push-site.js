...
//
// ゲート: 作業ツリー → 公開の有無 → 型検査の回帰テスト → 本番共通ビルド → 内部リンク切れ → 表示の崩れ → backlog の突き合わせ。
//
...
step("表示の崩れ（主要な幅での横はみ出し）");
if (run("node", ["tests/layout-check.js"], { stdio: "inherit" }).code !== 0) stop("横にはみ出しているページがあるか、ブラウザで検査できませんでした");
