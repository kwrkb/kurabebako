...
- 監視対象はすべてこのサイト（`https://kurabebako.com/`）です。実行区分の対象は、作成から削除までを
  ...
  監視対象になっているこのサイトの配信構成は、[静的サイトの無料ホスティング比較](/posts/static-site-hosting-free-tier/)で扱っています
- 「最短間隔」は無料枠で設定できる最小値です。有料プランの値は「料金」列に書いています
- この記事が扱うのは「サイトが応答しているか」です。「サイトがどれだけ読まれているか」を数えるサービスは、[アクセス解析の無料枠比較](/posts/analytics-free-tier/)で比べています
