...
## Phase 1 — コンテンツの型を決める ← **現在地**

1本目を書く前に、量産に頼らず勝つための「型」を確定させる。
`VISION.md` の差別化仮説（料金体系・制約・前提条件の粒度で上回る）を、
再現可能なテンプレートに落とす段階。

- [ ] 動作確認用のサンプル記事の扱いを決める（削除 / 差し替え）
- [ ] 比較記事のテンプレートを決める（見出し構成、比較表の列、出典の示し方）
- [ ] 扱う対象の線引きを具体化する（AIが実行・検証できるとは何を指すか）
- [ ] shortcode を実装する（`cta` / `ranking` / `pr` / 案件ID管理）
- [ ] PR表記の運用ルールを決める

**完了条件**: 同じ型で2本目以降を書き始められること

---

## Phase 2 — 投入とインデックス確認（〜3ヶ月）
